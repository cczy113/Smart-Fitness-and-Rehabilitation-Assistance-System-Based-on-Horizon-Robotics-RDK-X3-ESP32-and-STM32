...
本目录是为了 GitHub / NodeHub 网页上传整理出的开源上传版，已经剔除或拆分单文件超过 25MB 的内容。

### 大文件分片

以下原始大文件被拆分到了 `large_file_parts/`，每个分片不超过 20MiB，适合通过 GitHub 网页逐个上传：

- `小程序/wechat_devtools_2.01.2510290_win32_x64.exe`
- `肌电检测分支最终版/esp32_emg_firmware/esp32_emg_firmware.zip`

分片清单在：

```text
large_file_parts/manifest.json
```

如需还原这些大文件，在仓库根目录执行：

```powershell
powershell -ExecutionPolicy Bypass -File .\large_file_parts\restore_large_files.ps1
```

说明：

- 微信开发者工具安装包不是项目源码必需文件，推荐在 README 中放官方下载地址，不推荐长期放进 Git 仓库。
- `esp32_emg_firmware.zip` 是 ESP32 工程的压缩包，仓库中已经保留源码目录，通常不需要还原该 zip。
- 开源仓库建议保留源码、文档、协议和必要脚本，避免提交 `.pio/`、`.venv/`、`__pycache__/`、Keil `Objects/` 等生成文件。
## 开源说明

建议开源前补充：

- 项目许可证，例如 MIT、Apache-2.0 或 GPL。
- 硬件接线图和实物图。
- RDK 系统镜像/TROS 版本说明。
- Unity 项目仓库或通信协议链接。
- 微信小程序真实 AppID 不要提交到公开仓库，建议保留 `touristappid` 或使用示例配置。
...
每次修改 RDK、STM32、ESP32 或小程序后，建议在该文件末尾追加记录，包含修改人、现象、原因判断、改动内容、验证结果和遗留问题。
